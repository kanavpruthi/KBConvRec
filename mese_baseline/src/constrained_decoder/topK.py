...
from constrained_decoder.lexical_constraints import ConstrainedHypothesis, ConstrainedCandidate
from constrained_decoder.look_ahead import _generate_beam_search, _generate_greedy, _generate_sample
...
def _reorder_cache(past: Tuple, beam_idx: torch.Tensor) -> Tuple[torch.Tensor]:
    ...
    for layer_past in past:
        x = torch.stack((layer_past[0],layer_past[1]))
        y = x.index_select(1,beam_idx)
        ret.append(y)
    return tuple(ret)
